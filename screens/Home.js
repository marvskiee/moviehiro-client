...
import { setFavorites, setTempInfo } from "../redux";
...
const Home = (props) => {
  ...
  const custom_axios = axios.create({
    baseURL: "https://qhs78742sb.execute-api.us-west-2.amazonaws.com/",
  });
  const load = async () => {
    setIsLoading(true);
    setHasError(false);
  ...
  const Indicator = ({ scrollX, scrollTo }) => {
    return (
      <View>
        ...
        >
          {allData?.movie_links.slice(0, 15)?.map((_, i) => {
            const inputRange = [
              (i - 2) * width,
              (i - 1) * width,
              i * width,
        ...
        <>
          {hasError ? (
            ...
            <ScrollView>
              <Animated.FlatList
                ref={slidesRef}
                data={allData?.movie_links?.slice(0, 15)}
                horizontal
                  ...
                  return (
                    <View style={styles.carousel}>
                      ...
                      >
                        ...
                        >
                          <Text style={styles.episode_text}>{item?.type}</Text>
                          {item?.ratings && (
              ...
              <CardList
                title="Now Playing"
                data={allData?.now_playing_links}
                {...props}
              />
              <CardList
                title="Top Rated"
                data={allData?.top_rated_links}
                {...props}
              />
              <CardList
                title="Upcoming Movie's"
                data={allData?.upcoming_links}
                {...props}
              />
              <View
                style={{
...
export default connect(null, { setFavorites, setTempInfo })(Home);
